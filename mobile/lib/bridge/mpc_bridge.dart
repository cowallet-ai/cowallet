...
class MpcBridge {
  ...
  /// Load a raw 32-byte backup shard back into the Party 2 in-memory slot so it
  /// can be exported after an app relaunch (Rust memory is empty on cold start).
  /// Used by the mandatory post-rotation backup screen with the shard staged in
  /// SecureStorage during reshare.
  ///
  /// NOTE: `frb.loadBackupShardForExport` is generated by
  /// `flutter_rust_bridge_codegen generate` from the Rust
  /// `load_backup_shard_for_export`. Regenerate bindings after pulling the Rust
  /// change, or this call will not resolve.
  static Future<void> loadBackupShardForExport(List<int> backupBytes) async {
    if (backupBytes.length != 32) {
      throw MpcException('Backup shard must be 32 bytes');
    }
    try {
      await frb.loadBackupShardForExport(
        backupBytes: Uint8List.fromList(backupBytes),
      );
    } catch (e) {
      throw MpcException('Failed to load backup shard for export: $e');
    }
  }
