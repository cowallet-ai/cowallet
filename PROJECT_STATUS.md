...
> ⚠️ **MPC 安全修复上线须知** — 本批修复了 `sign.rs` 私钥泄露(严重)+ reshare 公钥绑定。上线有两个硬性约束:①App 与 Server 必须**同版本发布**(线路格式变更,跨版本反序列化会失败);②旧代码已签过名的**存量钱包私钥可能已泄露,reshare 救不了(不改公钥),需评估全新 DKG + 资产迁移**。详见 [`docs/RELEASE-NOTES-MPC-SECURITY-FIX.md`](docs/RELEASE-NOTES-MPC-SECURITY-FIX.md)。
