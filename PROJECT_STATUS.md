...
| 原生代币转账 | 🧪 已验证 | 🧪 已验证 | ETH/BNB/POL/MATIC 等原生币转账（MPC签名成功，BSC提交已修复） |
...
| 多链余额查询 | 🧪 已验证 | 🧪 已验证 | Covalent API 聚合查询所有链余额 |
| 交易历史记录 | 🧪 已验证 | 🧪 已验证 | 多链交易记录，ERC-20 Transfer 事件解析 |
...
| 特性 | 状态 | 说明 |
|------|------|------|
| 生物识别签名 | ✅ 已实现 | 🧪 已验证 | 指纹/Face ID 确认转账 |
