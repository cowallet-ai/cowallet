...
fn main() {
    ...
    dkg0.process_round1(vec![ProtocolMessage {
        session_id: "dkg".into(),
        from: 1,
        to: 0,
        round: 1,
        payload: r1_1.payload,
    }])
    .unwrap();
    dkg1.process_round1(vec![ProtocolMessage {
        session_id: "dkg".into(),
        from: 0,
        to: 1,
        round: 1,
        payload: r1_0.payload,
    }])
    .unwrap();
    ...
    let mut server_sign =
        SignSession::new_distributed(server_sign_config, share1.clone(), msg_hash_extracted);
    ...
    let server_sig_payload = server_sign
        .get_server_response()
        .expect("no server response");
