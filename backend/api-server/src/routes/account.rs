use axum::{extract::State, http::StatusCode, routing::delete, Router};
...
mod integration_tests {
    ...
    async fn count_user_rows(
        ...
        total += sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM email_verifications WHERE email = $1",
        )
        .bind(email)
        .fetch_one(pool)
        .await
        .unwrap();
        total += sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM login_challenges WHERE device_id = $1",
        )
        .bind(device_id)
        .fetch_one(pool)
        .await
    ...
    async fn purge_removes_all_rows_for_user(pool: sqlx::PgPool) {
        ...
        assert!(
            count_user_rows(&pool, uid, "alice@example.com", "device-alice").await >= 17,
            "seed should have populated every table"
        );

        purge_user_data(&pool, uid, "device-alice")
            .await
    ...
    async fn purge_leaves_other_users_untouched(pool: sqlx::PgPool) {
        ...
        purge_user_data(&pool, alice, "device-alice")
            .await
            .expect("purge must succeed");

        assert_eq!(
            count_user_rows(&pool, alice, "alice@example.com", "device-alice").await,
            0
        );
        // Bob is fully intact — no over-broad deletes.
        assert!(
            count_user_rows(&pool, bob, "bob@example.com", "device-bob").await >= 17,
